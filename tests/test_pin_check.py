...
def test_the_vendored_mpich_satisfies_the_pin_pypalace_declares():
    assert pin_check.satisfies(MPICH_VERSION, pin_check.PYPALACE_MPICH_REQUIREMENT)
...
def test_dependencies_are_read_from_a_pypalace_checkout(tmp_path):
...
def test_a_pypalace_pin_spelled_differently_still_agrees():
    assert pin_check.declared_problem("mpich<5", "mpich <5") is None


def test_a_pypalace_pin_that_moved_is_reported():
...
def test_a_pypalace_that_dropped_mpich_is_reported():
...
def test_the_command_line_check_passes_against_the_recorded_pin(capsys):
    ...
    # Without a checkout the check cannot see pypalace's side, and must say so
    # rather than report a comparison it did not make.
    assert "NOT compared against pypalace" in printed
...
def test_the_command_line_check_names_the_checkout_it_compared_against(
    ...
    assert pin_check.main(["--pypalace", str(tmp_path)]) == 0
...
def test_the_command_line_check_fails_against_a_diverged_checkout(tmp_path, capsys):
    ...
    assert pin_check.main(["--pypalace", str(tmp_path)]) == 1
